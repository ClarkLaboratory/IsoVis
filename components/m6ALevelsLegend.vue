...
Component to render an m6A modification levels legend plot, based on m6aLevelData which must be supplied as input.

<template>
<div>
    <div id="m6aLevelsLegendDiv">
        <p>m6A modification levels legend</p>
    </div>
</div>
</template>
...
export default {
    ...
    methods: {
        buildHeatmapLegend() {
            ...
            d3.select('#m6aLevelsLegendDiv').selectAll('*').remove();

            let el = document.getElementById("heatmapDiv");
            if (!(el && this.m6aLevelData)) return;

            let data = JSON.parse(JSON.stringify(this.m6aLevelData.samples));
            data.splice(this.m6aLevelData.location_colnum, 1);
            ...
            d3.select("#m6aLevelsLegendDiv").append("canvas").attr("id", "fontSizeCalcCanvas");
            ...
            d3.select("#m6aLevelsLegendDiv").append("canvas").attr("id", "heightCalcCanvas");
            ...
            d3.select("#m6aLevelsLegendDiv").append("canvas")
                .attr("width", Math.ceil(width))
                .attr("height", Math.ceil(height))
                .attr("id", "m6ALevelsLegendCanvas");
            let canvas = document.getElementById("m6ALevelsLegendCanvas");
            ...
            let min = this.m6aLevelData.minValue;
            let mid = this.m6aLevelData.average;
            let max = this.m6aLevelData.maxValue;
